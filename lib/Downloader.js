...
class Downloader {
    ...
    init() {
        ...
        let configFilePath = config.configPath + "/noaa-ww3.hjson";
        if (!fs.existsSync(configFilePath)) {
            fs.copyFileSync("./lib/res/noaa-ww3-sample-config.hjson", configFilePath);
        }
        ...
        this.downloadFile(file)
            ...
            .catch(error => {
                ...
                if (f2.retries > config.nRetries) {
                    ...
                    log.warn("Error downloading file '" + f2.url + "': " + error.toString() + ". Retry " + f2.retries + "/" + config.nRetries + " ... waiting 2 seconds");
                    f2.status = "pending";
                    this.setState(state);
                    setTimeout(_ => this.startNextDownload(), 2000);
                }
            })
    }
